...
# Nama kota-kota yang tersedia dan koordinat tetap
cities_coords = {
    "Kuala Lumpur": (3, 10),
    "Pahang": (8, 6),
    "Kelantan": (9, 9),
...
st.title("Coordinates for Cities")
...
colors = sns.color_palette("pastel", len(cities_coords))
...
if st.button("Submit"):
    ...
    fig, ax = plt.subplots()
    ax.grid(False)
    for i, (city, (city_x, city_y)) in enumerate(cities_coords.items()):
        color = colors[i]
        ...
        for j, (other_city, (other_x, other_y)) in enumerate(cities_coords.items()):
            if i != j:
    ...
    def dist_two_cities(city_1, city_2):
        city_1_coords = cities_coords[city_1]
        city_2_coords = cities_coords[city_2]
    ...
    def crossover(parent_1, parent_2):
        n_cities_cut = len(cities_coords) - 1
    ...
    best_population = run_ga(list(cities_coords.keys()), n_population, n_generations, crossover_per, mutation_per)
    ...
    x_best_route = [cities_coords[city][0] for city in best_route] + [cities_coords[best_route[0]][0]]
    y_best_route = [cities_coords[city][1] for city in best_route] + [cities_coords[best_route[0]][1]]
